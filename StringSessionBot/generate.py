...
ERROR_MESSAGE = "Oops! An exception occurred! \n\n**Error** : {} " \
            "\n\nPlease visit @Tellybots_support if this message doesn't contain any " \
...
async def generate_session(bot, msg, telethon=False):
    ...
    text = "**{} STRING SESSION** \n\n`{}` \n\nGenerated by @tellystringsessiongeneratorbot".format("TELETHON" if telethon else "PYROGRAM", string_session)
    await client.send_message("me", text)
    await client.disconnect()
    await phone_code_msg.reply("Successfully generated {} string session. \n\nPlease check your saved messages! \n\nGenerated by @Tellybots_4u".format("telethon" if telethon else "pyrogram"))
